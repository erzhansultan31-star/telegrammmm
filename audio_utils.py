...
def make_thumbnail(cover_bytes: bytes, max_side: int = 320) -> bytes:
    """Telegram-ның sendAudio thumbnail талаптарына сай кіші JPEG жасайды
    (<=320px, JPEG, <200KB). Ендірілген APIC-тен бөлек — Telegram
    чатта мұқабаны бірден көрсету үшін керек."""
    img = Image.open(io.BytesIO(cover_bytes)).convert("RGB")
    if max(img.size) > max_side:
        scale = max_side / max(img.size)
        img = img.resize((int(img.width * scale), int(img.height * scale)), Image.LANCZOS)
